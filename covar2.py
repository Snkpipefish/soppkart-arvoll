...
R=6378137.0
T=np.load('data/terrain.npz'); elev=T['elev']; slope=T['slope']
cell=(MX1-MX0)/N*math.cos(math.radians(59.963))
def ll2px(lat,lon,n):
...
tre=classify(rgb('data/sr16_SRRTRESLAG.png'),{(82,176,56):1,(205,170,101):2,(255,220,130):3})
bon=classify(rgb('data/sr16_SRRBONITET.png'),dict(zip(ramp[:8],[6,8,11,14,17,20,23,26])))
hgt=classify(rgb('data/sr16_SRRHOYDEM.png'),dict(zip(ramp,[1,3,5,7,9,11,13,15,16.5,19])))   # m
kro=classify(rgb('data/sr16_SRRKRONEDEK.png'),dict(zip(ramp,[5,15,25,35,45,55,65,75,85,95])))
vol=classify(rgb('data/sr16_SRRVOLMB.png'),{(204,236,230):50,(153,216,201):150,(102,194,164):250,(44,162,95):350,(0,109,44):450})
...
# ---------- Sentinel-2
def idx(name):
    d=np.load(f'data/s2_{name}.npz'); f=lambda b: np.maximum(d[b],1.0)
...
# true-colour image for display (13 Sep 2026)
rgbim=np.dstack([dla['red'],dla['green'],dla['blue']]); v=np.clip(rgbim/1300.0,0,1)**0.55
Image.fromarray((v*255).astype(np.uint8)).save('data/s2_truecolor.png')
# ---------- OSM rasters
hw=json.load(open('data/osm_highways.json'))['elements']; lu=json.load(open('data/osm_landuse.json'))['elements']
...
arv=json.load(open('arvollveien.json'))['elements']
...
np.savez_compressed('data/covars.npz',f_gran=f_gran,f_furu=f_furu,f_lauv=f_lauv,bon=bon_d,hgt=hgt_d,kro=kro_d,vol=vol_d,forest=forest,
