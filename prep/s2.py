...
"""Sentinel-2-bånd til rutenettet. Bruk: python prep/s2.py <områdemappe> scl <scene-id...> | python prep/s2.py <områdemappe> load <navn> <scene-id>"""
A=sys.argv[1]; D=f"{A}/data"; sys.argv=sys.argv[1:]
...
feats={f['id']:f for fn in [f'{D}/stac_spring.json',f'{D}/stac_summer.json'] for f in json.load(open(fn))['features']}
...
with rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN='EMPTY_DIR',GDAL_HTTP_UNSAFESSL='YES',GDAL_HTTP_MAX_RETRY=4,GDAL_HTTP_RETRY_DELAY=2):
    if mode=="scl":
        ...
        np.savez_compressed(f"{D}/s2_{name}.npz",**d)
